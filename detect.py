...
logging.basicConfig(level=logging.INFO, format='[%(relativeCreated)d] %(asctime)s :: %(message)s')
logging.info('Starting')

if 'PUSHBULLET_API_KEY' not in os.environ:
    raise 'PUSHBULLET_API_KEY not defined in env variables'
...
excluded_classes = ['train', 'umbrella', 'kite', 'boat', 'zebra']
...
#for path in glob.glob('/tmp/w*'):
    ...
    try:
        ...
        for idx, val in enumerate(scores[0]):
            ...
            if val > THRESHOLD:
                if class_name in excluded_classes:
                    ...
                    logging.warning('Found at index {} class: {} with {:.0f}% boxes: {}'.format(idx, class_name, val * 100, boxes[0][idx]))
        ...
        if len(current_classes) > 0:
            ...
            latest_notification = send_notification('Detected: {}'.format(detection_msg), datetime.now().strftime('%T'),
                    notification_host + outpath, PUSHBULLET_API_KEY, latest_notification)
